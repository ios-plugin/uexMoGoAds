...
- (void)getAd
{
    ...
    if ([_timeInterval isKindOfClass:[NSNumber class]]) {
        ...
        timer = [NSTimer scheduledTimerWithTimeInterval:AdapterTimeOut5 target:self selector:@selector(loadAdTimeOut:) userInfo:nil repeats:NO] ;
    }
}
 ...
 *  广告展示成功
 */
- (void)splashSuccessPresentScreen:(BaiduMobAdSplash *)_splash
{
    [self stopTimer];
    [self adSuccess:_splash];
}
 ...
 *  广告展示结束
 */
- (void)splashDidDismissScreen:(BaiduMobAdSplash *)_splash
{
    [self.splashAds adSplash:self didDismiss:_splash];
}
